...
echo "<link rel='stylesheet' href='Loading.css'>";
	$db = new mysqli("localhost","root","");
	 ...
	 $db->query("CREATE DATABASE IF NOT EXISTS `employee`");  // this line checks if the database has been created if not it create a database
 
	         mysqli_select_db($db,"employee"); //this line select the created database
              ...
              while($row1=mysqli_fetch_assoc($result4))
              {
                     $j=$j+1; 
                       ...
                       $j=(int)$j;
                        if($j==0)
                        {
                          $sql = "INSERT INTO `list_1`(`Username`, `Age`, `paswword`, `Adress`,`Contact`) VALUES ('$name','$age','$password','$adress','$contact')";
                          ...
                          echo "<h2 style='color:red'>Signup Unsuccessful<br>";
                          echo "Please enter Unique credentials</h2>";
                        }
                        $conn->close();
?>
